#ifndef SLING_H
#define SLING_H
/* Header-only Sling: lexer + parser + interpreter + runtime
...
static inline void setgpio(int pin, int level) {
    pinMode(pin, OUTPUT);
...
static inline void error(int ln, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    Serial.printf("[ERROR] line %d: ", ln);
    Serial.vprintf(fmt, ap);
    Serial.println();
    va_end(ap);

    // Instead of exit, just stop execution
    while (true) {
        delay(1000);  // freeze so ESP32 doesn’t reset
    }
}
...
static inline void lex(const char *src) {
    ...
    while (*p) {
        ...
        if (*p == '!') { add_token("NOT"); strcpy(last_token,"NOT"); p++; continue; }
...
static inline Value eval(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case AST_FUNCTION_CALL: {
            ...
            if (strcmp(node->funccall.funcname, "gpio") == 0) {
                if (node->funccall.arg_count == 1) {
                    // ✅ gpio(pin) → read value
                    int pin = (int)eval(node->funccall.args[0]).number;
                    pinMode(pin, INPUT_PULLUP);        // use internal pull-up resistor
                    int val = digitalRead(pin);

                    return (Value){ .type = VAL_NUMBER, .number = val };
                }
                else if (node->funccall.arg_count == 2) {
                    // ✅ gpio(pin, value) → write value
                    int pin   = (int)eval(node->funccall.args[0]).number;
                    Value val = eval(node->funccall.args[1]);

                    int level = LOW;
                    if (val.type == VAL_STRING) {
                        if (strcasecmp(val.string, "HIGH") == 0) level = HIGH;
                        else if (strcasecmp(val.string, "LOW") == 0) level = LOW;
                        free(val.string);
                    } else {
                        level = (val.number != 0) ? HIGH : LOW;
                    }
                    ...
                    error(0, "gpio() requires 1 (read) or 2 (write) arguments");
                    return (Value){ .type = VAL_NUMBER, .number = 0 };
                }
            }
            if (strcmp(node->funccall.funcname, "printlog") == 0) {
                if (node->funccall.arg_count < 1) {
                    error(0, "printlog() requires 1 argument");
                    return (Value){ .type = VAL_NUMBER, .number = 0 };
                }
                char *msg = eval_to_string(node->funccall.args[0]);
                Serial.println(msg);   // print to Arduino console
                free(msg);
                return (Value){ .type = VAL_NUMBER, .number = 1 };
            }
...
static inline void run_script(const char *src) {
    ...
    lex(src);
    if (i == 0) { error(0, "No tokens generated"); return; }

    ASTNode *root = parse_statements();
    if (!root) { error(0, "Failed to parse source"); return; }

    interpret(root);
    free_ast(root);

}
static inline void run_script_string(const char *script) {
    if (!script || !*script) {
        error(0, "No script string provided");
        return;
    }
    ...
    lex(script);
    if (i == 0) {
        error(0, "No tokens generated from script string");
        return;
    }
    ...
    ASTNode *root = parse_statements();
    if (!root) {
        error(0, "Failed to parse script string");
        return;
    }
